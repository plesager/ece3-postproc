...
*** Requirements
    The acceptance/reproducibility test relies on a set of scripts written in
    R. Few R packages are also needed: /s2dverification, ncdf4,
    RColorBrewer/. If you do not control you environment and R and/or the
    packages are missing, it may be easier to work on another machine where
    you can easy installed the packages for example:

    : # define a personal R library location, and make sure that R is aware of it: 
...
*** Experiment design
    ...
    conditions. For AMIP runs, you can create these initial conditions on the
    fly, by adding a call to the perturbation script in your
    /classic/ece-*.sh.tmpl/, i.e. by replacing:
    : ln -s \
    ...
    For CMIP, you need to have the perturbed ocean restart ready. TO BE CONTINUED.
    
    Your experiments name should be made of *3 characters (the stem) followed
    by a number from 1-to-5*. For example: /cca1, cca2, cca3, cca4, cca5/. The
    stem uniquely defines your ensemble.
...
*** Postprocessing steps
    ...
    from each run into a tar file in your $SCRATCH. This is useful for sharing
    and then being able to compare with another ensemble results.
 
*** Comparing 
    Once you have two ensembles, you can compare them. Two parameters must be
    set in the /compare.sh/ script:
    - CLIMDIR: location of EC-mean climatology sets for all runs
    - RK: location of R&K performance indices tables for the ensembles
    ...
    previous step), RK and CLIMDIR are both the directory where you unpack the
    archive.
