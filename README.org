...
*** Tools settings
    Each tool has its specifics settings. If the code has already been ported
    to your machine, you will have to change very little there, as described
    hereafter. They define the needed executable/lib (cdo, nco, netcdf,...),
    location of auxiliary data, and where to save the results. For reference,
...
*** First, run HIRESCLIM2 to extract and average variables of interest:
    ...
    that you may have to change are the templates for the model output and for
    ...
    in EC-Earth3 output. If needed, change the variable name from EC-Earth in
    //hiresclim2/script/nemo_post.sh/ below:
