...
** INSTALLATION
   ...
   of the EC-Earth model data, mrundir and post-processing dir, must be known.
   ...
   : export ECE3_POSTPROC_RUNDIR=<dir of your ecearth run -see your config-run.xml>
   : export ECE3_POSTPROC_POSTDIR=<dir to store temporary post-processed files (hiresclim2)>
   ...
   For defining the rundir, postdir and diagdir variables you can use the
   following environment variables which will be expanded when running the
   scripts, make sure you enclose the with single-quotes.
   ...
   : export ECE3_POSTPROC_DIAGDIR='$SCRATCH/ecearth-runs/$EXPID/diag'
